...
const hexadecimalAHsv = (hexadecimal) => {
	...
	const r =Math.round(parseInt(hexadecimal.substring(1,3), 16) /255 * 1000) / 1000;
	const g = Math.round(parseInt(hexadecimal.substring(3,5), 16) /255 * 1000) / 1000;
	const b = Math.round(parseInt(hexadecimal.substring(5,7), 16) /255 * 1000) /1000;
...
const rangoColor = (color) => {
	...
	if(h>=20 && h<50 && v>=brilloMinimo && v<0.6 && s > 0.4) return "Marrón";
