...
const validarFormulario = (
	...
	for (let i = 0; i < elementosFormulario.length; i++) {
		if (
			elementosFormulario[i].tagName === "INPUT" ||
			elementosFormulario[i].tagName === "SELECT"
		) {
			if (!campoValido(elementosFormulario[i])) {
				valido = false;
				if (errores && nombreClaseError !== "") {
					//Por como lo he diseñado los errores van de uno en uno.
...
const mostrarError = (elementoError, errores, nombreClaseError) => {
	...
	if (
		elementoError.nextSibling &&
		elementoError.nextSibling.classList.contains(nombreClaseError)
	) {
		const divError = elementoError.nextSibling;
