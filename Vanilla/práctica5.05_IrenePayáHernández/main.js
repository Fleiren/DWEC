"use strict";
import { fechaActual, validarCampo, validarFormulario, mostrarErrores } from "./biblioteca/utilFormularios.js";
window.onload = () => {
    ...
    fechaActual(anyo);
    //Lo he separado en dos eventos porque los select no funcionan con input y si pongo los input en el evento change se ponene en rojo cuando pierden el foco y no es lo que quiero.
    formularioDisco.addEventListener("input", (evento) => {
        if(evento.target.tagName === "INPUT"){
            validarCampo(evento.target) ? evento.target.classList.remove("error") : evento.target.classList.add("error");
        }
    }, false);
    formularioDisco.addEventListener("change", (evento) => {
        if(evento.target.tagName==="SELECT"){
            //Le colocamos la clase de error si no es válido.
            validarCampo(evento.target) ? evento.target.classList.remove("error") : evento.target.classList.add("error");
        }
    }, false);
    formularioDisco.addEventListener("click", (evento) => {
