...
const useSupabaseCRUD = (tableName) => {
	...
	if(!tableName || typeof tableName !== "string"){
		throw new Error("El nombre de la tabla está sin definir, debes indicarla para poder realizar consultas.")
	}
	...
	const save = async (item) => {
		request(supabaseConnexion.from(tableName).insert(item));
	};
	...
	const remove = async (id) =>{
		return await request(supabaseConnexion.from(tableName).delete().eq("id", id));
	}
