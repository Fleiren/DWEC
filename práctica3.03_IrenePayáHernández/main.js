...
let discente = {
	...
	imprimirAficiones: function() {
		console.log(`Aficiones:\n${this.aficiones.map((aficion) => `- ${aficion}.`).join("\n")}`);
	},
	imprimirInforme: function() {
		console.log("- Informe del discente -")
		console.log(`Id: ${this.id}, es un dato de tipo ${typeof this.id}.`);
		console.log(`Nombre: ${this.nombre}, es un dato de tipo ${typeof this.nombre}.`);
		console.log(`Apellidos: ${this.apellidos}, es un dato de tipo ${typeof this.apellidos}.`);
		...
		console.log(`Notas:\n${Object.entries(this.notas).map(([key, value]) => `- ${key}: ${value.toLocaleString("es-ES")}`).join("\n")} \nEs un dato de tipo ${typeof discente.notas}.`);
		//Aquí uso replace porque no me funcionaba bien el método toLocaleString.
		console.log(`Media: ${this.calcularMedia().toLocaleString("es-ES")} es un dato de tipo ${typeof this.calcularMedia()}.`);
...
let objetoComplejo = {
	...
	temas: ["Redención y esperanza frente a la desesperación.", "Política y religión como herramientas de poder.", "Magia y sus limitaciones", "Superación personal y liderazgo"],
	personajes: [{
		nombre: "Raoden",
